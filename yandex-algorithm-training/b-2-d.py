...
как оно задано во входных данных. Ножки следует перечислять слева направо,
в том порядке, в котором они встречаются во входных данных.
"""
from typing import List


def binary_search(a: List[int], num: int) -> int:
    left = 0
    right = len(a) - 1
    while left <= right:
...
def solution(blocks: List[int], length: int) -> List[int]:
    i = binary_search(blocks, length // 2)
    if i == 0:
        ans = [blocks[i]]
    elif i == len(blocks):
        ans = [blocks[i - 1]]
...
def main():
    length, _ = (int(num) for num in input().split())
    blocks = [int(num) for num in input().split()]
    print(*solution(blocks, length))
